...
###### We wanted to:
- Create a working UI.
- Have the API server commmunicate with the simulation engine.
- Come up with a design specification for the oracle.
...
Since our client is built in React (browser based) the most idiomatic way to stream real-time data from the simulation-engine to the UI was via web-sockets. However, as Golang does not offer a very straightforward way to setup a websocket service, and also due to the fact that we wanted to keep a separation of concerns, we decieded to create an additional service written in Java using the Spring framework to sit inbetween the UI and simulation-engine, referred to as api-server.
...
- Proposed dockerisation of the systems for easier deployments.
...
- Disease regulations (e.g. vaccination programs).

**Next Steps:**
- Finalise the UI prototype and integrate with backend sytems for user testing.
...
- **Git Issues:** Problem with merging causing some deletion of files. 

- **Plan:** Revert to previous commit to recover files.

- **GitHub Access:** Problems with access resolved.
...
- Prioritising issues in the GitHub for better visibility of team progress.
